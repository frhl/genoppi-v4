...
test_that('calc_hyper can return correct overlap results',{
  ...
  # InWeb
  result <- calc_hyper(sigDf,"InWeb",inwebDf,T,"BCL2")
  expect_equal(format(result[[1]]$pvalue,digits=3),"0.908")
  expect_equal(result[[2]]$successInSample_genes,"LARP1")

  # gene list
  result <- calc_hyper(sigDf,"ALS",geneDf,intersectN,"BCL2")
  expect_equal(format(result[[1]]$pvalue,digits=3),"0.00513")
  expect_identical(sort(result[[2]]$successInSample_genes),c("ATXN2","FUS","PFN1","TAF15"))
