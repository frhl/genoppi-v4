...
test_that('get_gwas_lists can return correct data.frame',{
  ...
  expect_identical(sort(as.character(unique(result[[1]]$trait))),traits)
  expect_identical(sort(as.character(unique(result[[1]]$gene))),c("C9orf72","DACH1","LIPC","SUSD1"))
  expect_equal(sort(unique(result[[2]]$PUBMEDID)),
