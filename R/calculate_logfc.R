#' @title calculate log fold change (LogFC)
#' @description calculates log fold change by substracting a case column with a bait column. Will apply
#' to all numeric entries with the rep1-9 in the column names
#' @param df a data.frame with bait and controls
#' @author flassen
#' @note assummes log2(bait1), log2(control1), log2(bait2).. 
#' @export

calculate_logfc <- function(df){
